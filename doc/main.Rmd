...
However, in fact that it would be actually better if we are using the matching dictionaries with the confusion matrix to generate the `potential candidates list`. `Associated Press` is the one used in the paper. Therefore, we also generate potential candidates using `Associated Press` as a souce. This dictionary provides with more meaning corrections that suit the everyday use.

```{r}
# To keep consistency, we also generated the candidate list that is outputed by using AP dictionary
load('../output/typos.RData')
## For each typo, list the candidate
...
# Note that some of the typos cannot find a meaningful(listed in the dictionary) correction.
candidates_ap <- candidates
save(candidates_ap,file='../output/candidates.RData') # possible correction by AP
```
...
load('../output/tesseract_correction.RData')
...
tesseract_delete_error_vec <- str_split(tesseract_correction," ")[[1]]%>%
  sapply(.,tolower)%>%
  as.vector()%>%
  gsub('[[:punct:] ]+','',.)
