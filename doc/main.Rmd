...
```{r eval=FALSE}
# For every typo, apply potential_correction function and for each candidate, compute the probability

## Create a list that contains typo and position in tesseract_vec
## This is done by ../lib/typo position.R
load('../output/typos.RData')

## For each typo, list the candidate
source('../output/potential correction.R')
...
for(i in seq(1,nrow(typos),1000))
{
  j <- ifelse((i+1000-1)<=nrow(typos),i+1000-1,nrow(typos))
  for(k in i:j)
  {
    temp <- potential_correction(typos[k,1])
...
save(candidates,file='../output/candidates.RData')
```
...
for(i in seq(1,nrow(typos),1000))
{
  ...
  # print(i) This is used to record the process
}
# Note that some of the typos cannot find a meaningful(listed in the dictionary) correction.
candidates_ap <- candidates
save(candidates_ap,file='../output/candidates_ap.RData') # possible correction by AP
```
